...
#ifndef lint
static char rcsid[] __attribute__ ((unused)) = "$Header: /usr/cvsroot/magic-7.5/dbwind/DBWdisplay.c,v 1.11 2008/02/10 19:06:32 tim Exp $";
#endif  /* not lint */
    ...
    TerminalPath *tpath;	/* Contains pointer to full pathname of label */
{
    ...
    if (label->lab_flags & PORT_DIR_MASK)
        GrSetStuff(STYLE_PORT);
	
    DBWDrawLabel(label, &labRect, screenPos, -1,
