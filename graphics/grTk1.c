    ...
    XEvent *xevent;
{
	...
	case ButtonPress:
		...
		entry = HashLookOnly(&grTkWindowTable, wind);
    ...
    char *name;
{
	...
	entry = HashFind(&grTkWindowTable, tkwind);
    ...
    MagWindow *w;
{
    ...
    entry = HashLookOnly(&grTkWindowTable, xw);
    ...
    char *tkname;
{
    ...
    if (tkwind != NULL)
    {
	entry = HashLookOnly(&grTkWindowTable, tkwind);
	mw = (entry) ? (MagWindow *)HashGetValue(entry) : 0;
	if (mw) id = mw->w_wid;
    }
    return id;
