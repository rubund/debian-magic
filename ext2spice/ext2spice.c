...
#ifndef lint
static char rcsid[] __attribute__ ((unused)) = "$Header: /usr/cvsroot/magic-7.5/ext2spice/ext2spice.c,v 1.11 2008/12/02 17:12:35 tim Exp $";
#endif  /* not lint */
...
#endif
int spcmainArgs();
int spccapVisit(), spcdevVisit(), spcnodeVisit(), subcktVisit(), spcresistVisit(), 
    devMergeVisit(), devDistJunctVisit();
int subcktUndef();
...
static unsigned short esFormat = SPICE3 ;
    ...
    TxCommand *cmd;
{
	...
	case EXTTOSPC_SUBCIRCUITS:
	    ...
	    idx2 = Lookup(cmd->tx_argv[3], yesno);
	    switch (idx2) {
		case IDX_YES: case IDX_TRUE: case IDX_ON:
		    if (idx == IDX_DESCEND)
			esDoPorts = TRUE;
...
runexttospice:
    ...
    (void) sprintf( esSpiceCapFormat, "C%%d %%s %s %%.%dlffF%%s", resstr, esCapAccuracy);
    ...
    Def *def;
{
	...
	while (he = HashNext(&def->def_nodes, &hs))
	{
	    ...
	    if (snode->efnode_name->efnn_node->efnode_flags & EF_PORT)
    ...
    Transform *trans;	/* Coordinate transform for output */
{
	...
	default:
	    if (dev->dev_nterm < 2)
	    {
		TxError("Device other than a subcircuit has only "
			"one terminal\n");
		return 0;
	    }
	    break;
