...
#ifndef lint
static char rcsid[] __attribute__ ((unused)) = "$Header: /usr/cvsroot/magic-7.5/database/DBtcontact.c,v 1.10 2008/09/04 13:10:37 tim Exp $";
#endif  /* not lint */
	...
	else if (!strcmp(*argv, "stackable"))
	{
		...
		else
		{
			...
			else if (lastType >= TT_SPACE)
			{
				...
				DBTechAddNameToType(*argv, newType, TRUE);
			    lastType = TT_SPACE;
			}
			else
			{
