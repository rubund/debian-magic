...
#ifndef lint
static char rcsid[] __attribute__ ((unused)) = "$Header: /usr/cvsroot/magic-7.5/database/DBcellsrch.c,v 1.7 2010/08/15 14:35:27 tim Exp $";
#endif  /* not lint */
    ...
    struct scaleArg *scvals;
{
    ...
    DBNMPaintPlane(scvals->ptarget, exact, &targetRect,
		...
		(PaintUndoInfo *)NULL);
#else
    DBPaintPlane(scvals->ptarget, &targetRect,
		((scvals->doCIF) ? CIFPaintTable :
		DBStdPaintTbl(type, scvals->pnum)),
		(PaintUndoInfo *)NULL);
#endif
