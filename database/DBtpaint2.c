...
#ifndef lint
static char rcsid[] __attribute__ ((unused)) = "$Header: /usr/cvsroot/magic-7.5/database/DBtpaint2.c,v 1.8 2009/12/30 13:45:48 tim Exp $";
#endif  /* not lint */
 ...
 *
 *	Create a mask of unique types left over when the type of lpErase
 *	is erased from the type of lpImage.  The rules are as follows:
 *
 *	1) If lpErase is a non-contact type, the mask contains all contact
 *	   types whose residues are subsets of the "have" type but are not
 *	   supersets of the "erase" type.
 *	2) If lpErase is a contact type, the mask contains all contact types
 *	   whose residues are subsets of the "have" type but do not share
 *	   more than one plane with the "erase" type.
 *	3) All types that are subsets of other types in the mask are removed.
 *
 * Results:
 *	None.
 *
    ...
    TileTypeBitMask *outMask;
{
	...
	if (TTMaskEqual(&smask, &li->l_residues))
	{
	    ...
	    if (lpErase->l_isContact)
	    {
		/* The residues of type li cannot share more than one	*/
		/* plane with the residues of the erase type		*/

		for (t = TT_TECHDEPBASE; t < DBNumUserLayers; t++)
		    if (TTMaskHasType(&smask, t))
		    {
			TTMaskClearType(&smask, t);
			break;
		    }

		if (!TTMaskIsZero(&smask)) continue;
	    }
	    else
	    {
		/* The residues of type li cannot be a superset of the	*/
	    ...
	    /* Remove any types that are a subset of this type		*/
	    /* Ignore this type if it is a subset of another type	*/
	    /* Ignore this type if it overlaps another type and	there	*/
	    /*	 is no stacking type for the two.			*/

	    for (m = 0; m < dbNumContacts; m++)
	    {
		lc = dbContactInfo[m];
		if (TTMaskHasType(outMask, lc->l_type))
		{
		    cmask = lc->l_pmask & li->l_pmask;
		    if (cmask == lc->l_pmask)	
			TTMaskClearType(outMask, lc->l_type);
		    else if (cmask == li->l_pmask)	
		    {
			TTMaskClearType(outMask, li->l_type);
			break;
		    }
		    else if (cmask != 0)
		    {
			stype = DBTechFindStacking(lc->l_type, li->l_type);
			if (stype < 0)
			{
			    TTMaskClearType(outMask, li->l_type);
			    break;
...
{
    int pNum;
    PlaneMask pmask, overlap;
    LayerInfo *lp;
    TileTypeBitMask cmask;
    TileType itype, stype, ctype;
    ...
    dbComposeSubsetResidues(lpImage, lpErase, &cmask);
    ...
    for (itype = DBNumUserLayers; itype < DBNumTypes; itype++)
	if (TTMaskHasType(&cmask, itype))
	{
	    lp = &dbLayerInfo[itype];
	    SETERASE(lpImage->l_type, lpErase->l_type, DBPlane(lp->l_type), itype);
	}
