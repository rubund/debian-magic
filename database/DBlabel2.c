...
#ifndef lint
static char rcsid[] __attribute__ ((unused)) = "$Header: /usr/cvsroot/magic-7.5/database/DBlabel2.c,v 1.1.1.1 2006/04/10 22:03:14 tim Exp $";
#endif  /* not lint */
    ...
    ClientData cdarg;		/* Argument to pass to function */
{
    ...
    if (DBTreeSrLabels(scx, mask, xMask, &tpath, dbSrLabelFunc,
		(ClientData) &labsr))
	return 1;
    else return 0;
}
				 ...
				 */
{
    ...
    (void) DBTreeSrLabels(&scx, &DBAllTypeBits, xMask, tp, dbNearestLabelFunc,
	(ClientData) &funcData);
