...
#ifndef lint
static char rcsid[] __attribute__ ((unused)) = "$Header: /usr/cvsroot/magic-7.5/database/DBpaint.c,v 1.14 2010/06/24 19:57:50 tim Exp $";
#endif  /* not lint */
...
//	/* Sounded good, but violates maximum horiz. strips!	*/
//	/* Beware violating maximum horiz. strips property	*/
//	/* unless you want to change the code in a lot of	*/
//	/* places. . .						*/
//
//	if (oldType == newType)
//	     goto paintdone; /* nothing to do */
	...
	if (oldType != newType)
	{
		...
		if (IsSplit(tile))
		{
		    ...
		    /* Ensure maximum horizontal strips for non-split-tile areas */
		    tp = TR(newtile);
		    if (!IsSplit(tp))
		        /* Merge the outside tile to its right if possible */
			if (TiGetTypeExact(TR(tp)) == TiGetTypeExact(tp))
			    dbPaintMerge(tp, TiGetTypeExact(tp), plane,
					MRG_RIGHT, undo);
		    tp = newtile;
		    if (!IsSplit(tp))
		        /* Merge the outside tile to its right if possible */
			if (TiGetTypeExact(BL(tp)) == TiGetTypeExact(tp))
			    newtile = dbPaintMerge(tp, TiGetTypeExact(tp), plane,
					MRG_LEFT, undo);
		}
		else
		{
		...
		if (IsSplit(tile))
		{
		    ...
		    if (!IsSplit(tp))
		        /* Merge the outside tile to its right if possible */
			if (TiGetTypeExact(TR(tp)) == TiGetTypeExact(tp))
			    dbPaintMerge(tp, TiGetTypeExact(tp),
					plane, MRG_RIGHT, undo);
		    tp = newtile;
		    if (!IsSplit(tp))
		        /* Merge the outside tile to its right if possible */
			if (TiGetTypeExact(TR(tp)) == TiGetTypeExact(tp))
			    newtile = dbPaintMerge(tp, TiGetTypeExact(tp),
					plane, MRG_RIGHT, undo);
		}
		else
		{
	...
	if (newType & TT_DIAGONAL)
	{
		...
		/* Reinstate the left merge requirement */
		mergeFlags |= MRG_LEFT;
	    }
	    else
		mergeFlags = 0;
 ...
 * existing tiles which cross the area boundary.  After fracturing,
 * it merges tiles where possible inside and outside the area boundary,
 * but retains the boundary.  This boundary will be used by DBNMPaintPlane
 * to paint a non-manhattan diagonally split tile.  This routine does no
 * painting, except where sub-splitting diagonal tiles requires a change
 * in tile type.
 *
...
enumerate:
	...
	/* These merges are opposite to what is done in the	*/
	/* DBPaintPlane routine.				*/

	mergeFlags = 0;
	if (LEFT(tile) > area->r_xbot) mergeFlags |= MRG_LEFT;
	if (RIGHT(tile) < area->r_xtop) mergeFlags |= MRG_RIGHT;
	if (TOP(tile) < area->r_ytop) mergeFlags |= MRG_TOP;
	if (BOTTOM(tile) > area->r_ybot) mergeFlags |= MRG_BOTTOM;
	...
	if (TOP(tile) > area->r_ytop)
	{
		...
		if (!IsSplit(tile)) oldType = TiGetTypeExact(tile);

		/* Ensure maximum horizontal strips for non-split-tile	*/
		/* areas outside of the fracture zone.			*/
...
paintmerge:
	...
	{
	    for (tp = BL(tile); BOTTOM(tp) < TOP(tile); tp = RT(tp))
		if (TiGetTypeExact(tp) == oldType)
		{
		    tile = dbPaintMerge(tile, oldType, plane, mergeFlags, undo);
		    goto paintdone;
		}
	    mergeFlags &= ~MRG_LEFT;
	}
 ...
 * dbMergeNMTilesFunc --
 *
 *	Procedure to detect when an area of non-Manhattan tiles can be 
 *	merged together into a single, larger non-Manhattan tile.
 *
 * Results:
 *	Return 1 if any splitting was done.
 *
 * Side Effects:
...
int
dbMergeNMTilesFunc(tile, cdata)
    Tile *tile;
    ClientData cdata;
{
    mergeNMStruct *nmstruct = (mergeNMStruct *)cdata;
    Plane *plane = nmstruct->plane;
    PaintResultType *resultTbl = nmstruct->resultTbl;
    Tile *tp;
    int aspecta, aspectb;
    bool tileOK = FALSE;
    Rect rect;
    TileType ttype, htype;
    TileTypeBitMask tmask;

    int dbcUnconnectFunc();	/* forward reference */
    ...
    {
	/* find tile at NW corner */
	tp = RT(tile);
	while (LEFT(tp) >= LEFT(tile))
	    tp = BL(tp);
	if ((RIGHT(tp) == LEFT(tile)) && (BOTTOM(tp) == TOP(tile)))
	    tileOK = TRUE;
    }
    else
    {
	/* find tile at NE corner */
	tp = RT(tile);
	tp = TR(tp);
	while (BOTTOM(tp) > TOP(tile))
	    tp = LB(tp);
	if ((RIGHT(tile) == LEFT(tp)) && (BOTTOM(tp) == TOP(tile)))
	    tileOK = TRUE;
    }
    if (!IsSplit(tp) || !tileOK || (TiGetTypeExact(tp) != ttype)
			|| (SplitDirection(tp) != SplitDirection(tile)))
	return 0;
    ...
    TTMaskSetOnlyType(&tmask, TiGetLeftType(tile));
    TTMaskCom(&tmask);
    if (SplitDirection(tile) == 0)
    {
	rect.r_xbot = LEFT(tile);
	rect.r_xtop = RIGHT(tile);
	rect.r_ybot = TOP(tile);
	rect.r_ytop = TOP(tp);
    }
    else
    {
	...
	rect.r_xtop = LEFT(tile);
	rect.r_ytop = BOTTOM(tp);
    }

    /* Check for correct material in opposite corners	*/
    /* Use dbcUnconnectFunc, which just returns 1 for	*/
    /* tiles of the wrong type.				*/

    if (DBSrPaintArea(tile, plane, &rect, &tmask, dbcUnconnectFunc,
		(ClientData)NULL))
	return 0;

    TTMaskSetOnlyType(&tmask, TiGetRightType(tile));
    TTMaskCom(&tmask);
    ...
    TxPrintf("non-Manhattan merge possible at %d %d to %d %d\n",
		rect.r_xbot, rect.r_ybot, rect.r_xtop, rect.r_ytop);

    /* Erase the area, then paint back with ttype		*/
    /* This is a non-operation as far as "undo" is concerned	*/

    /* No, this is no good because using DBPaintPlane means we	*/
    /* are not guaranteed that the result is a single tile in	*/
    /* the area we want to merge.  We need to merge the tiles	*/
    /* inside the area ourselves.				*/

    htype = TiGetLeftType(tile);
    if (htype == TT_SPACE)
	htype = TiGetRightType(tile);

    DBPaintPlane(plane, &rect, resultTbl, (PaintUndoInfo *)NULL);

    tile = plane->pl_hint;
    GOTOPOINT(tile, &(rect.r_ll));
 ...
 * DBUnFracturePlane --
 *
 *	DBFracturePlane() specifically prohibits merging across the
 *	fracture boundary.  Therefore, for tiles that are otherwise
 *	untouched by the DBNMPaintPlane() routine, we need to complete
 *	the merging to ensure the integrity of the maximum-horizontal-
 *	stripes database.
 *
 * Results:
 *	None.
 ...
 *
 * Notes:
 *	This routine should check for simple cases where two
 *	non-Manhattan tiles can be merged into one larger tile.
 *	Procedure:  Find the two tiles at the endpoints of the
 *	tile's diagonal.  If either one is a non-Manhattan tile
 *	with the same (exact) type and tile aspect ratio (same
 *	angle of the diagonal), then check the other two areas
 *	for all-left-type and all-right-type.  If this check
 *	passes, erase the area and paint a single non-Manhattan
 *	tile.  Or is it easy enough to do the merges & splits?
 *
 * ----------------------------------------------------------------------------
 */
...
void
DBUnFracturePlane(plane, area, undo)
    Plane *plane;		/* Plane whose paint is to be modified */
    Rect *area;			/* Area to be changed */
    PaintUndoInfo *undo;	/* Record containing everything needed to
				 * save undo entries for this operation.
				 ...
				 */
{
    ...
    /*
     * Increase the search area by 1 in all directions to catch tiles
     * on the outside of the fracture boundary.
     */
    areaex.r_xbot = area->r_xbot - 1;
    areaex.r_ybot = area->r_ybot - 1;
    areaex.r_xtop = area->r_xtop + 1;
    areaex.r_ytop = area->r_ytop + 1;
    ...
    start.p_x = areaex.r_xbot;
    start.p_y = areaex.r_ytop - 1;
    tile = plane->pl_hint;
    GOTOPOINT(tile, &start);

    /* Each iteration visits another tile on the LHS of the search area */
    /* Note that this iteration visits tiles all tiles that are inside	*/
    /* OR WHICH TOUCH the search area.  This catches tiles generated by	*/
    /* TiNMSplit(X,Y)() that were outside of the search area but need	*/
    /* to be merged into the surrounding area.				*/

    while (TOP(tile) > areaex.r_ybot)
    {
	/***
	 *** AREA SEARCH.
	 *** Each iteration enumerates another tile.
...
enumerate:
	...
	clipTop = TOP(tile);
	if (clipTop > areaex.r_ytop) clipTop = areaex.r_ytop;

	/* At this point, we should incorporate the routine	*/
	/* dbMergeNMTilesFunc(), or at least its intended	*/
	/* effect.						*/
	...
	mergeFlags = MRG_LEFT | MRG_RIGHT | MRG_TOP | MRG_BOTTOM;
	ttype = TiGetTypeExact(tile);
	...
	if (mergeFlags & MRG_LEFT)
	{
		...
		{
		    tile = dbPaintMerge(tile, ttype, plane, mergeFlags, undo);
		    goto paintdone;
		}
	    mergeFlags &= ~MRG_LEFT;
	}
...
paintdone:
	/* Move right if possible */
	tpnew = TR(tile);
	if (LEFT(tpnew) < areaex.r_xtop)
	{
	    /* Move back down into clipping area if necessary */
	    while (BOTTOM(tpnew) >= clipTop) tpnew = LB(tpnew);
	    if (BOTTOM(tpnew) >= BOTTOM(tile) || BOTTOM(tile) <= areaex.r_ybot)
	    {
		tile = tpnew;
		goto enumerate;
	    }
	...
	/* Each iteration returns one tile further to the left */
	while (LEFT(tile) > areaex.r_xbot)
	{
	    /* Move left if necessary */
	    if (BOTTOM(tile) <= areaex.r_ybot)
		goto done;
	...
	/* At left edge -- walk down to next tile along the left edge */
	for (tile = LB(tile); RIGHT(tile) <= areaex.r_xbot; tile = TR(tile))
	    /* Nothing */;
    }
...
void
DBNMPaintPlane(plane, exacttype, area, resultTbl, undo)
    Plane *plane;		/* Plane whose paint is to be modified */
...

{
	...
	/* Break the plane up around the specified boundary */
	DBFracturePlane(plane, area, resultTbl, undo);
	...
	(void) DBSrPaintArea((Tile *) NULL, plane, area,
		&DBAllTypeBits, dbNMEnumFunc, (ClientData) &lhead);

	/*--------------------------------------------------------*/
	/* If there's only one tile underneath, we can just paint */
	/* it with the diagonal.  Otherwise, we subsplit the area */
	/* and call this routine recursively until the area does  */
	/* not fracture any further.				  */
	/*--------------------------------------------------------*/
	...
	while (lr != NULL)
	{
	    ...
	    /* Clip to area:  DBFracturePlane() is supposed to do this, */
...
paintrect:
		...
		DBNMPaintPlane(plane, exacttype, &(lr->r_r), resultTbl, undo);
	    }
...
nextrect:
	...
	/* Restore the maximum horizontal strips policy on the	*/
	/* plane around the fractured boundary.			*/
...
// nmmerge:
//
//  /* Walk the area, looking for non-Manhattan merges.	*/
//  /* This is experimental code, disabled for now.	*/
//  /* Also see "goto nmmerge" line, above.		*/
//
//  nmstruct.plane = plane;
//  nmstruct.resultTbl = resultTbl;
//
//  while (DBSrPaintArea((Tile *) NULL, plane, area, &DBAllTypeBits,
//	dbMergeNMTilesFunc, (ClientData) &nmstruct));
 ...
 *	Modifies the database plane that contains the given tile.
 *
 * NOTE:  The purpose of this routine is to avoid passing over areas twice.
 * For the DRC interaction check, processing areas twice generates a false
 * positive error by painting error_s paint.
 *
 * ----------------------------------------------------------------------------
 */
...
/*----------------------------------------------------------------------*/
/* Currently, the non-manhattan version is non-functional, other than	*/
/* to declare the proper parameters to match DBNMPaintPlane().  The	*/
/* non-manhattan functionality will need to be added for proper DRC	*/
/* checking of non-manhattan geometry.					*/
/*----------------------------------------------------------------------*/

void
DBNMPaintPlaneMergeOnce(plane, exacttype, area, resultTbl, undo)
    Plane *plane;		/* Plane whose paint is to be modified */
    TileType exacttype;		/* Source tile type diagonal info (only) */
    Rect *area;	/* Area to be changed */
    PaintResultType *resultTbl;	/* Table, indexed by the type of tile already
				 * present in the plane, giving the type to
				 * which the existing tile must change as a
    ...
    PaintUndoInfo *undo;	/* Undo record */
{
    ...
    delx = (y - BOTTOM(*oldtile)) * (RIGHT(*oldtile) - LEFT(*oldtile));
    haschanged = (x = (delx % height) << 1) ? ((undo) ? TRUE : FALSE) : FALSE;
    x = (x >= height) ? 1 : 0;
    delx /= height;
    delx += x;
    ...
    if (undo) DBPAINTUNDO(*oldtile, TiGetTypeExact(*oldtile), undo);
    ...
    PaintUndoInfo *undo;	/* Undo record */
{
    ...
    dely = (x - LEFT(*oldtile)) * (TOP(*oldtile) - BOTTOM(*oldtile));
    haschanged = (y = (dely % width) << 1) ? ((undo) ? TRUE : FALSE) : FALSE;
    y = (y >= width) ? 1 : 0;
    dely /= width;
    dely += y;
