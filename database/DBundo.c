...
#ifndef lint
static char rcsid[] __attribute__ ((unused)) = "$Header: /usr/cvsroot/magic-7.5/database/DBundo.c,v 1.4 2008/01/14 23:31:35 tim Exp $";
#endif  /* not lint */
    ...
    paintUE *up;
{
    ...
    if ((up->pue_newtype == up->pue_oldtype) && (up->pue_newtype & TT_DIAGONAL))
	return;		/* ignore 4-way splits on undo forward */
    ...
    paintUE *up;
{
    ...
    if ((up->pue_newtype == up->pue_oldtype) && (up->pue_newtype & TT_DIAGONAL))
    {
	/* Paint TT_SPACE into area to remove any internal fractures */
	DBPaintPlane(dbUndoLastCell->cd_planes[up->pue_plane], &up->pue_rect,
			DBStdPaintTbl(TT_SPACE, up->pue_plane),
			(PaintUndoInfo *) NULL);
    }
 ...
 */
	...
	int	 lue_pos;	/* Relative position of label */
	TileType lue_type;	/* Type of tile labelled */
	unsigned int lue_flags;	/* Flags for label */
...
void
DBUndoPutLabel(cellDef, rect, pos, text, type, flags)
    CellDef *cellDef;	/* CellDef being modified */
    Rect *rect;/* Box definining label.  The lower left coordinate
...
void
DBUndoEraseLabel(cellDef, rect, pos, text, type, flags)
    ...
    labelUE *up;
{
	...
	up->lue_text, up->lue_type, up->lue_flags);
